...
router.put("/:id", validateToken, async (req, res) => {
  ...
  try {
    ...
    if (num == 1) {
      res.json({
        message: "ChatArea was updated successfully.",
      });
    } else {
      res.status(400).json({
        message: `Cannot update chatarea with id ${id}.`,
      });
    }
  } catch (err) {
    res.status(400).json({ errors: err.errors });
...
router.delete("/:id", validateToken, async (req, res) => {
  ...
  let num = await ChatArea.destroy({
    where: { id: id },
  });
  if (num == 1) {
    res.json({
      message: "ChatArea was deleted successfully.",
    });
  } else {
    res.status(400).json({
      message: `Cannot delete chatarea with id ${id}.`,
    });
  }
});
