...
function EditChat() {
  ...
  useEffect(() => {
    http.get(`/chatarea/${id}`).then((res) => {
      setPost(res.data);
      setImageFile(res.data.images?.[0]);
      setLoading(false);
    });
  }, [id]);
  ...
  const formik = useFormik({
    ...
    validationSchema: yup.object({
      ...
      link: yup.string().url("Enter a valid URL").nullable(true),
    }),
    onSubmit: (data) => {
      if (imageFile) {
        data.imageFile = imageFile; // Use imageFile key instead of images
      }
        ...
        <Box component="form" onSubmit={formik.handleSubmit}>
          <Grid container spacing={2}>
            <Grid item xs={12} md={6} lg={8}>
              ...
              <Editor
                apiKey="gttjrrjm8n7xtxw6pty3he9edaz1kb03g14ro3lubgg5ctpn"
