...
const { Report, ChatArea, User } = require("../models");
...
router.put("/:id", validateToken, async (req, res) => {
  try {
    ...
    // If the status is DELETED, first update the report, then delete the post
    if (status === "DELETED") {
      await report.update({ status });

      // Now delete the post
      const post = await ChatArea.findByPk(report.postId);
      if (post) {
        await post.destroy();
      }
    } else {
